...
static void do_Aget(std::vector<std::string> &cmd, std::string &out) {
	int rKey = stoi(cmd[1]);
	Node *node = tree->search(tree->root, rKey);
	if (node == NULL) {
...
static void do_request(std::vector<std::string> &cmd, std::string &out) {
	...
	if (cmd.size() == 1 && cmd_is(cmd[0], "keys")) {
		...
		do_del(cmd, out);
	} else if (cmd.size() == 3 && cmd_is(cmd[0], "aset")) {
		do_Aset(cmd, out);
	} else if (cmd.size() == 2 && cmd_is(cmd[0], "aget")) {
		do_Aget(cmd, out);
	} else if (cmd.size() == 2 && cmd_is(cmd[0], "adel")) {
		do_Adel(cmd, out);
	} else {
